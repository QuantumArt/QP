...
* Запросы в Common.cs большей частью переведены на параметрические запросы (когда параметр идентификатор или массив идентификаторов)  (#173095)
* Исправлена sql уязвимость. Строковые значения из форм переведены на параметрические запросы (Пользователи, Контенты, Поиск по коду, Аудит) (#173095)
* Исправлена sql уязвимость для customFilter. Это параметр в запросе на бэк для получения данных для гридов. Параметр customFilter теперь передается не в виде sql выражения, а в виде json документа, в котором описаны применяемые фильтры (#173095)
