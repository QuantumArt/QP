...
* Запросы в Common.cs большей частью переведены на параметрические запросоы (когда параметр идентификатор или массив идентификаторов)
* Исправлена sql уязвимость. Строковые значения из форм переведены на параметрические запросы (Пользователи, Контенты, Поиск по коду, Аудит)
* Исправлена sql уязвимость для customFilter. Теперь в customFilter передается не в виде sql выражения, а в виде json документа, в котором описанв применяемые фильтры
