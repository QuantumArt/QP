...
|----------------|----------|
| td_content_indexes | Срабатывает после удаления ограничения. Удаляет соответствующие индексы |
| tu_content_indexes | Срабатывает после обновления ограничения. Удаляет соответствующие индексы |
...
| Имя поля       | Описание |
|----------------|----------|
| td_content_item_schedule_del_job | Вызывается после удаления записи о расписании. Удаляетсоответствующий Job. |
...
| Имя триггера   | Описание |
|----------------|----------|
| tbd_content_item_version | Вызывается перед удалением версии. Выполняет необходимое каскадное удаление. |
...
| Имя поля       | Описание |
|----------------|----------|
| ti_union_contents_auto_map_attrs | Выполняется после вставки новых записей. Осуществляет автоматическое заполнение таблицы UNION_ATTRS. |
...
| Имя поля       | Описание |
|----------------|----------|
| td_content_and_article_workflow_bind | Выполняется после удаления Workflow. Удаляет привязки к контентам и статьям. |
| ti_access_workflow | Выполняется после создания Workflow. Добавляет права доступа по умолчанию на Workflow. |
