...
Идентификатор родительского элемента данных ([Таблица CONTENT_DATA](#таблица-content_data)). Используется механизмом вариаций.
...
ID последнего пользователя ([Таблица USERS](#таблица-users)), менявшего группу пользователей
...
Члены группы могут управлять задачами, выполняемыми по расписанию, не будучи администраторами
...
Идентификатор дочерней группы ([Таблица USER_GROUP](#таблица-user_group))

##### Parent_Group_Id

Идентификатор родительской группы ([Таблица USER_GROUP](#таблица-user_group))
...
Идентификатор группы ([Таблица USER_GROUP](#таблица-user_group))

##### USER_ID

Идентификатор пользователя ([Таблица USERS](#таблица-users))
...
Идентификатор уровня доступа ([Таблица PERMISSION_LEVEL](#таблица-permission_level))
...
ID последнего пользователя ([Таблица USERS](#таблица-users)), менявшего правило доступа
...
Идентификатор действия, для которого создается правило доступа ([Таблица BACKEND_ACTION](#таблица-backend_action))
...
Идентификатор контента, для которого создается правило доступа ([Таблица CONTENT](#таблица-content))
...
Идентификатор статьи, для которой создается правило доступа ([Таблица CONTENT_ITEM](#таблица-content_item))
...
Идентификатор Workflow, для которого создается правило доступа ([Таблица WORKFLOW](#таблица-workflow))
...
Идентификатор типа сущности, для которого создается правило доступа ([Таблица ENTITY_TYPE](#таблица-entity-type))
...
Идентификатор вкладки, для которой создается правило доступа ([Таблица TABS](#таблица-tabs))
...
ID контента, из которого берется фильтр ([Таблица CONTENT](#таблица-content))

##### CONTENT_ITEM_ID

ID статьи для фильтрации ([Таблица CONTENT_ITEM](#таблица-content_item))
...
Идентификатор версии статьи ([Таблица CONTENT_ITEM_VERSION](#таблица-content_item_version))
...
ID пользователя, выполнившего операцию ([Таблица USERS](#таблица-users))
...
ID пользователя, выполнившего действие ([Таблица USERS](#таблица-users))
...
ID авторизованного пользователя ([Таблица USERS](#таблица-users))
...
ID последнего пользователя ([Таблица USERS](#таблица-users)), менявшего профиль пользователя
...
Не используется (сохранено для обратной совместимости c QP7)
...
Настройки режима OnScreen в ASP. Устаревшее поле (сохранено для обратной совместимости c QP7)
...
Дата последней модификации пароля. Вычисляется триггером
...
Флаг, указывающий на то, что пользователь должен сменить пароль при следующем логине в систему
...
Таблица дополнительной аутентификации по токену, которая для своей работы должна быть включена настройкой USE_TOKENS в таблице DB
...
Дата последней транзакции, полученной через CDC
...
ID формата по умолчанию ([Таблица OBJECT_FORMAT](#таблица-object_format))
...
ID поля плагина QP ([Таблица PLUGIN_FIELD](#таблица-plugin_field))
