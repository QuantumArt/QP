...
{
    public sealed class CdcTarantoolContentToContentAsyncProcessor : BaseCdcTarantoolImportProcessor
    {
        public CdcTarantoolContentToContentAsyncProcessor(string captureInstanseName)
                    ...
                    return new CdcTableTypeModel
                    {
                        ...
                        Entity = new CdcEntityModel
                        {
                            ...
                            Columns = new Dictionary<string, object>
                            {
                                ...
                                { TarantoolContentToContentModel.InvariantName, TarantoolContentToContentModel.GetInvariantAsyncName(linkId, false) },
                                { TarantoolContentToContentModel.LeftContentId, Convert.ToInt32(row[ContentToContentColumnName.LContentId]) },
                                { TarantoolContentToContentModel.RightContentId, Convert.ToInt32(row[ContentToContentColumnName.RContentId]) },
                                { TarantoolContentToContentModel.IsSymmetric, (bool)row[ContentToContentColumnName.IsSymmetric] },
                                { TarantoolContentToContentModel.IsReverse, false },
