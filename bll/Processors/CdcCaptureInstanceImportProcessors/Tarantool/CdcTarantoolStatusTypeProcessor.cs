...
{
    public sealed class CdcTarantoolStatusTypeProcessor : BaseCdcTarantoolImportProcessor
    {
        public CdcTarantoolStatusTypeProcessor(string captureInstanseName)
                ...
                try
                {
                    return new CdcTableTypeModel
                    {
                        ...
                        Entity = new CdcEntityModel
                        {
                            ...
                            {
                                { TarantoolStatusTypeModel.StatusTypeId, Convert.ToInt32(row[StatusTypeColumnName.StatusTypeId]) },
                                { TarantoolStatusTypeModel.StatusTypeName, row[StatusTypeColumnName.StatusTypeName] as string }
                            }
                        }
                    };
                }
