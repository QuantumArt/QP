	...
	internal class UserMapper : GenericMapper<User, UserDAL>
	{
        ...
        public override void CreateDalMapper()
        {
            Mapper.CreateMap<User, UserDAL>()
                .ForMember(data => data.LanguageId, opt => opt.MapFrom(src => Utils.Converter.ToNullableDecimal(src.LanguageId)))
				...
				.ForMember(data => data.LastLogOn, opt => opt.MapFrom(biz => biz.LastLogOn == default(DateTime) ? (DateTime?)null : biz.LastLogOn))
				;
        }
