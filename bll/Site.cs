...
{ 
	[HasSelfValidation]
    public partial class Site : LockableEntityObject
	{
		...
		internal Site()
		{
			...
			ContextClassName = Site.DefaultContextClassName;
			ConnectionStringName = Site.DefaultConnectionStringName;
		...
		[FormatValidator(Constants.RegularExpressions.InvalidEntityName, Negated = true, MessageTemplateResourceName = "NameInvalidFormat", MessageTemplateResourceType = typeof(SiteStrings))]
		...
		[FormatValidator(Constants.RegularExpressions.NetName, MessageTemplateResourceName = "ConnectionStringNameInvalidFormat", MessageTemplateResourceType = typeof(SiteStrings))]
		...
		[FormatValidator(Constants.RegularExpressions.FullQualifiedNetName, MessageTemplateResourceName = "NamespaceInvalidFormat", MessageTemplateResourceType = typeof(SiteStrings))]
		...
		[FormatValidator(Constants.RegularExpressions.NetName, MessageTemplateResourceName = "ContextClassNameInvalidFormat", MessageTemplateResourceType = typeof(SiteStrings))]
		...
		public override string EntityTypeCode
		{
			get
			{
				return Constants.EntityTypeCode.Site;
			}
		}
		...
		public string IsLiveIcon
		{
			get
			{
				return String.Format("site{0}.gif", (IsLive) ? 1 : 0);
			}
		}
		...
		public string LongUploadUrl
		{
			get
			{
				string prefix = (UseAbsoluteUploadUrl) ? this.UploadUrlPrefix : "http://" + Dns;
		...
		public string TestBinDirectory
		{
			get
			{
				return Utils.PathUtility.Combine(TestDirectory, SitePathRepository.RELATIVE_BIN_PATH);
			}
		}
		...
		public string FullyQualifiedContextClassName
		{
			get
			{
				return Site.GetFullyQualifiedName(Namespace, ContextClassName);
			}
		}
		...
		{
			RulesException<Site> errors = new RulesException<Site>();
		...
		private void ValidateXaml(RulesException<Site> errors)
		{
			if (!String.IsNullOrWhiteSpace(XamlDictionaries))
			{
				...
				{
					errors.ErrorFor(f => f.XamlDictionaries, String.Format("{0}: {1}", SiteStrings.XamlDictionaries, exp.Message));
				}
			}
		}
		...
		{
			CorrectSlashMode dirMode = CorrectSlashMode.RemoveLastSlash | CorrectSlashMode.ReplaceDoubleSlashes | CorrectSlashMode.ConvertSlashesToBackSlashes;
			CorrectSlashMode urlMode = CorrectSlashMode.ReplaceDoubleSlashes | CorrectSlashMode.ConvertBackSlashesToSlashes | CorrectSlashMode.WrapToSlashes;
			CorrectSlashMode absUrlMode = CorrectSlashMode.ConvertBackSlashesToSlashes | CorrectSlashMode.RemoveLastSlash;
		...
		{
			DynamicResourceDictionaryContainer container = new DynamicResourceDictionaryContainer();			
			DynamicResourceDictionary dict = new DynamicResourceDictionary {Name = "Site"};
		...
		public void SaveVisualEditorCommands(int[] activeVeCommands)
		{
			...
			var oldSiteCommands = VisualEditorRepository.GetResultCommands(Id);// с этим нужно сравнивать на предмет измененеий

			var changedCommands = new Dictionary<int, bool>();
			...
			foreach (var cId in activeVeCommands)
			{
				if (!oldSiteCommands.SingleOrDefault(c => c.Id == cId).On)
					changedCommands.Add(cId, true);
			}
		...
		internal void SaveVisualEditorStyles(int[] activeStyleIds)
		{
			...
			var oldSiteStyles = VisualEditorRepository.GetResultStyles(Id);
			
			var changedStyles = new Dictionary<int, bool>();
			var defaultStyleDictionary = new Dictionary<int, bool>();

			foreach (var s in defaultStyles)
			{
				defaultStyleDictionary.Add(s.Id, s.On);
			}

			foreach (var cId in activeStyleIds)
			{
				if (!oldSiteStyles.SingleOrDefault(s => s.Id == cId).On)
					changedStyles.Add(cId, true);
			}

			foreach (var cId in offVeStyles)
			{
				if (oldSiteStyles.SingleOrDefault(c => c.Id == cId).On)
