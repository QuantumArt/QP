...
namespace Quantumart.QP8.BLL.Repository
{
	internal class EntityTypeRepository
	{
		...
		{
			EntityType entityType = MappersRepository.EntityTypeMapper.GetBizObject(DefaultEntityTypeQuery.Single(et => et.Code == entityTypeCode));
			return entityType;
		}
		...
		{
			return GetList()
					.Where(e => entityCodes.Contains(e.Code, StringComparer.InvariantCultureIgnoreCase))
					.ToArray();
		}
		...
		{
			EntityTypeDAL entityType = QPContext.EFContext.EntityTypeSet
				.Include("Parent")
				.SingleOrDefault(et => et.Code == entityTypeCode);
		...
		internal static string GetDefaultActionCodeByEntityTypeCode(string entityTypeCode)
		{
			...
			//QPContext.EFContext.GetDefaultActionCodeByEntityTypeCode(entityTypeCode);			
		}
	}
}
