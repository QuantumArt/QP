...
namespace Quantumart.QP8.BLL.Repository
{
    internal class WorkflowRepository
    {
		...
		internal static void UpdateContentWorkflowBind(ContentWorkflowBind binding)
		{
			...
			bool changed = persisted && needToPersist &&
				(oldDal.WorkflowId != binding.WorkflowId || oldDal.IsAsync != binding.IsAsync);
			...
			{
				DefaultRepository.SimpleDelete<ContentWorkflowBindDAL>(oldDal.EntityKey);
			}
			
			if (persisted && changed || !persisted && needToPersist)
			{
				DefaultRepository.SimpleSave<ContentWorkflowBindDAL>(newDal);
			}
