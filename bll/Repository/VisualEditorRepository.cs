...
{
	class VisualEditorRepository
	{
		internal static IEnumerable<VisualEditorPluginListItem> List(ListCommand cmd, int contentId, out int totalRecords)
		{
			using (var scope = new QPConnectionScope())
			{
				IEnumerable<DataRow> rows = Common.GetVisualEditorPluginsPage(scope.DbConnection, contentId, cmd.SortExpression, out totalRecords, cmd.StartRecord, cmd.PageSize);
		...
		internal static VisualEditorPlugin UpdatePluginProperties(VisualEditorPlugin plugin)
		{
			...
			VePluginDAL dal = MappersRepository.VisualEditorPluginMapper.GetDalObject(plugin);
		...
		private static void UpdateCommands(VisualEditorPlugin plugin, QP8Entities entities, DateTime timeStamp)
		{
			...
			//delete
			HashSet<decimal> newIds = new HashSet<decimal>(plugin.VeCommands.Select(c => Converter.ToDecimal(c.Id)));
			...
			//save and update
			Queue<int> forceIds = (plugin.ForceCommandIds == null) ? null : new Queue<int>(plugin.ForceCommandIds);
			foreach (var command in plugin.VeCommands)
			{
				VeCommandDAL dalCommand = MappersRepository.VisualEditorCommandMapper.GetDalObject(command);
		...
		internal static IEnumerable<VisualEditorCommand> GetResultCommands(int fieldId, int siteId)
		{
			...
			var fieldCommands = VisualEditorRepository.GetFieldCommands(fieldId);
		...
		internal static void SetSiteCommands(int siteId, Dictionary<int, bool> changedCommands, Dictionary<int, bool> defaultCommandsDictionary)
		{
			using (var scope = new QPConnectionScope())
			{
				...
				{
					if (defaultCommandsDictionary.ContainsKey(rec.Key) && defaultCommandsDictionary[rec.Key] == rec.Value)//совпадает с дефолтными настройками
					{
			...
			Dictionary<int, bool> siteCommandsDictionary)
		{
			using (var scope = new QPConnectionScope())
			{
				foreach (var rec in changedCommands)
				{
					if (siteCommandsDictionary.ContainsKey(rec.Key))//совпадает с настройками сайта
					{
					...
					{
						if (defaultCommandsDictionary.ContainsKey(rec.Key))//совпадает с дефолтными настройками
						{
		...
		{			
			using (var scope = new QPConnectionScope())
			{
				IEnumerable<DataRow> rows = Common.GetVisualEditorStylesPage(scope.DbConnection, contentId, cmd.SortExpression, out totalRecords, cmd.StartRecord, cmd.PageSize);
		...
		internal static IEnumerable<VisualEditorStyle> GetResultStyles(int fieldId, int siteId)
		{
			var defaultStyles = GetAllStyles();
			var siteStyles = GetSiteStyles(siteId);
			var fieldStyles = VisualEditorRepository.GetFieldStyles(fieldId);			
		...
		internal static void SetSiteStyles(int siteId, Dictionary<int, bool> changedStyles, Dictionary<int, bool> defaultStyleDictionary)
		{
			using (var scope = new QPConnectionScope())
			{
				...
				{
					if (defaultStyleDictionary.ContainsKey(rec.Key) && defaultStyleDictionary[rec.Key] == rec.Value)//совпадает с дефолтными настройками
					{
		...
		internal static void SetFieldStyles(int siteId, int fieldId, Dictionary<int, bool> changedStyles, Dictionary<int, bool> defaultStylesDictionary,
			Dictionary<int, bool> siteStylesDictionary)
		{
			using (var scope = new QPConnectionScope())
			{
				foreach (var rec in changedStyles)
				{
					if (siteStylesDictionary.ContainsKey(rec.Key))//совпадает с настройками сайта
					{
					...
					{
						if (defaultStylesDictionary.ContainsKey(rec.Key))//совпадает с дефолтными настройками
						{
		...
		internal static Dictionary<int, bool> GetStyleBindingByFieldId(int fieldId)
		{
			using (var scope = new QPConnectionScope())
			{
				return Common.GetStyleBindingByFieldId(scope.DbConnection, fieldId)
					.ToDictionary(r => Converter.ToInt32(r.Field<decimal>("STYLE_ID")), r => r.Field<bool>("ON"));
			}
		}
		...
		internal static Dictionary<int, bool> GetStyleBindingBySiteId(int siteId)
		{
			using (var scope = new QPConnectionScope())
			{
				return Common.GetStyleBindingBySiteId(scope.DbConnection, siteId)
					.ToDictionary(r => Converter.ToInt32(r.Field<decimal>("STYLE_ID")), r => r.Field<bool>("ON"));
			}
		}
