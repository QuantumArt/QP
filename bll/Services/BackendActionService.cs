...
namespace Quantumart.QP8.BLL.Services
{
	public class BackendActionService
	{
		...
		{
			BackendAction action = BackendActionRepository.GetById(actionId);
			return action != null ? action.Code : null;
		}
		...
		public static BackendAction GetByCode(string actionCode)
		{
			if (String.IsNullOrWhiteSpace(actionCode))
				...
				return BackendActionRepository.GetByCode(actionCode);
		}
		...
		{
			return ResolveStatusForCustomActions(actionCode, entityId, parentEntityId, 
						BackendActionRepository.GetStatusesList(actionCode, entityId)
				   );
		}
		...
		{			
			string etypeCode = GetByCode(actionCode).EntityType.Code;
			return CustomActionResolver.ResolveStatus(etypeCode, entityId, parentEntityId, statuses.ToArray());
		}
		...
		{
			return BackendActionRepository.GetInterfaceActionsForCustom()
				.GroupBy(a => a.EntityTypeId)
				.Select(g => new EntityTypeIdToActionListItemPair
				{
					EntityTypeId = g.Key,
