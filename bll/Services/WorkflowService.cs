...
namespace Quantumart.QP8.BLL.Services
{
	...
	{		
		...
		{
			int totalRecords;
			IEnumerable<WorkflowListItem> list = WorkflowRepository.GetSiteWorkflowsPage(cmd, siteId, out totalRecords);
		...
		{
			Workflow result = WorkflowRepository.SaveProperties(workflow);
			return result;
		}
		...
		{
			return SecurityRepository.IsEntityAccessible(Constants.EntityTypeCode.Content, contentId, Constants.ActionTypeCode.Update, userId);
		}

		public bool IsContentAccessibleForUserGroup(int contentId, int userGroupId)
		{
			return SecurityRepository.IsEntityAccessibleForUserGroup(Constants.EntityTypeCode.Content, contentId, Constants.ActionTypeCode.Update, userGroupId);
		}
