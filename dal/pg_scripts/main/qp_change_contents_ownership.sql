...
AS $BODY$
    ...
    BEGIN
        ...
        from (select tablename from pg_catalog.pg_tables where tablename SIMILAR TO 'content_[\d]+[\w_]*') t
        loop
        ...
        from (select viewname from pg_catalog.pg_views where viewname SIMILAR TO 'content_[\d]+[\w_]+') v
        loop
            execute myrow.viewq;
        end loop;
    END;
