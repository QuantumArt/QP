...
BEGIN
	...
	insert into @ids2 select id from @ids i inner join content_item ci with(nolock) on i.ID = ci.CONTENT_ITEM_ID and SCHEDULE_NEW_VERSION_PUBLICATION = 1

	if exists(select * From @ids2)
	begin
		exec qp_merge_links_multiple @ids2
