...
namespace System.Linq.Dynamic
{
    ...
    {
        public static IQueryable<T> Where<T>(this IQueryable<T> source, string predicate, params object[] values) => (IQueryable<T>)Where((IQueryable)source, predicate, values);
        ...
        public static IQueryable<T> OrderBy<T>(this IQueryable<T> source, string ordering, params object[] values) => (IQueryable<T>)OrderBy((IQueryable)source, ordering, values);
    ...
    public static class DynamicExpression
    {
        ...
        public static LambdaExpression ParseLambda(Type itType, Type resultType, string expression, params object[] values) => ParseLambda(new[] { Expression.Parameter(itType, "") }, resultType, expression, values);
    ...
    public sealed class ParseException : Exception
    {
        ...
        public override string ToString() => string.Format(Res.ParseExceptionFormat, Message, Position);
    }
...
#pragma warning restore 0219
        ...
        private Expression ParseMemberAccess(Type type, Expression instance)
        {
            ...
            return info != null ? Expression.Property(instance, info) : Expression.Field(instance, (FieldInfo)member);
        }
        ...
        private Exception IncompatibleOperandsError(string opName, Expression left, Expression right, int pos) => ParseError(pos, Res.IncompatibleOperands,
        ...
        private Expression GenerateNotEqual(Expression left, Expression right) => Expression.NotEqual(left, right);
        ...
        private Expression GenerateSubtract(Expression left, Expression right) => Expression.Subtract(left, right);

        private Expression GenerateStringConcat(Expression left, Expression right) => Expression.Call(
            null,
            typeof(string).GetMethod("Concat", new[] { typeof(object), typeof(object) }),
            new[] { left, right });

        private MethodInfo GetStaticMethod(string methodName, Expression left, Expression right) => left.Type.GetMethod(methodName, new[] { left.Type, right.Type });

        private Expression GenerateStaticMethodCall(string methodName, Expression left, Expression right) => Expression.Call(null, GetStaticMethod(methodName, left, right), new[] { left, right });
        ...
        private bool TokenIdentifierIs(string id) => _token.id == TokenId.Identifier && string.Equals(id, _token.text, StringComparison.OrdinalIgnoreCase);
        ...
        private Exception ParseError(string format, params object[] args) => ParseError(_token.pos, format, args);

        private static Exception ParseError(int pos, string format, params object[] args) => new ParseException(string.Format(CultureInfo.CurrentCulture, format, args), pos);
