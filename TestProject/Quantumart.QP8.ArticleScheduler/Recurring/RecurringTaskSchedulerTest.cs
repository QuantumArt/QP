...
namespace WebMvc.Test.Quantumart.QP8.ArticleScheduler.Recurring
{
	...
	public class RecurringTaskSchedulerTest
	{
		...
		public void Initialize()
		{
			...
			task = new RecurringTask(204, 1633, RecurringTaskTypes.MonthlyRelative, 9, 4, 5,
		...
		{			
			...
			new RecurringTaskScheduler(vssMock.Object, operationsLogWriter).Run(task);
		...
		public void RunTest_CurrentDatetimeWithoutVisibileRange_HideArticle()
		{
			...
			// скрыть статью 			
			vssMock.Verify(f => f.ShowArticle(It.IsAny<int>()), Times.Never());
			vssMock.Verify(f => f.HideArticle(task.ArticleId), Times.Once());
			vssMock.Verify(f => f.HideAndCloseSchedule(It.IsAny<int>()), Times.Never());
		}
