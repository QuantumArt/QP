...
namespace Quantumart.QPublishing.Controls
{
    public class QUserControlEssential
    {
        ...
        public string TraceObjectString { get; set; } = string.Empty;

        public string UndefTraceString { get; set; } = string.Empty;
        ...
        public void AddQpDefValue(string key, object value)
        {
            if (key != null)
            {
                ...
                if (QpDefValues.ContainsKey(key))
                {
                    QpDefValues.Remove(key);
                }
