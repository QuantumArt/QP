    ...
    public class TranslateManager
    {
        ...
        public static string Translate(string phrase, bool forJavaScript)
        {
            return HttpContext.Current.Session["CurrentLanguageID"].ToString() == "1" ? 
                phrase : GetTranslation(int.Parse(GetPhraseId(phrase)), (int)HttpContext.Current.Session["CurrentLanguageID"], phrase);
        }
        ...
        public static string GetTranslation(int phraseId, int languageId, string phraseText)
        {
            ...
            {
                var dt = conn.GetCachedData(
                    $"select * from translations where phrase_id = {phraseId} and language_id = {languageId}");
