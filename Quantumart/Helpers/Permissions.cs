...
namespace Quantumart.QPublishing.Helpers
{
    public class Permissions
    {
        ...
        {
            var conn = new DBConnector();
            const string selectClause = "select gg.*, pg.groupName as parent_group_name, cg.groupName as child_group_name " + " from group_to_group as gg " + " inner join user_group as pg on pg.groupId = gg.parentGroupId " + " inner join user_group as cg on cg.groupId = gg.childGroupId ";
            return conn.GetCachedData(selectClause);
        }
