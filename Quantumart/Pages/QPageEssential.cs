...
namespace Quantumart.QPublishing.Pages
{
    ...
    public class QPageEssential
    {
        ...
        private string GetViewFieldValue(DataView view, string fieldName)
        {
        ...
        byte includeArchive, long lngUserId, long lngGroupId, int intStartLevel, int intEndLevel)
        {
        ...
        internal void ShowObject(string name, object sender, object[] parameters, IQPage page, bool useSimpleInitOrder)
        {
            ...
            {
                if (ControlExists(inputName)) throw;
        ...
        private void AddToCallStack(string name)
        {
            ...
            if (ObjectCallStack.Count > 32) CallStackOverflow(); 
        }
        ...
        private void AppendControl(object sender, Control ctrl)
        {
        ...
        {
            if (QpTrace != null) InitControlTrace<T>(ctrl); 
            ((T)ctrl).UseSimpleInitOrder = useSimpleInitOrder;
            ((T)ctrl).OnLoadControl(sender, ref name);
            if (!useSimpleInitOrder) ((T)ctrl).SimulateOnInit(new EventArgs()); 
            
            AppendControl(sender, ctrl);
            
            if (QpTrace != null) EndControlTrace<T>(ctrl); 
        }
        ...
        private bool IsTemplateObject(DataRowView drv)
        {
            return ReferenceEquals(drv["PAGE_ID"], DBNull.Value);
        }
        
        private string GetNetTemplateName(DataRowView drv)
        {
        ...
        internal Hashtable PageObjects => _pageObjects ??
                                          (_pageObjects =
                                              IsPreview || IsStage
                                                  ? Cnn.CacheManager.FillPageObjectsHashTable()
                                                  : Cnn.GetPageObjectHashTable());

        private Hashtable _templateObjects;
        internal Hashtable TemplateObjects => _templateObjects ??
                                              (_templateObjects =
                                                  IsPreview || IsStage
                                                      ? Cnn.CacheManager.FillTemplateObjectsHashTable()
                                                      : Cnn.GetTemplateObjectHashTable());
        ...
        public void HandleInit(IQPage qPage)
        {
            ...
            if (GenerateTrace) InitTrace();
            if (IsStage && !IsPreview) EnableBackendOnScreen(qPage, site_id); 
        ...
        {
            var publisher = "Quantum Art's QP7.Framework 7.7";
        ...
        public void EnableBackendOnScreen(IQPage qPage, int siteId)
        {
            if (QScreen.SessionEnabled()) {
                ...
                if (qPage.site_id > 0) QScreen.SetSiteBorderModes(Cnn.GetSite(siteId)); 
            }
        }
        ...
        public void AddValue(string key, object value)
        {
            if (key != null) {
                ...
                if (key != null && _valuesCollection.ContainsKey(key)) _valuesCollection.Remove(key);
                if (key != null) _valuesCollection.Add(key, value);
            }
        }
        ...
        {
            key = Strings.LCase(key);
            if (key != null && _objectValuesCollection.ContainsKey(key)) _objectValuesCollection.Remove(key);
            if (key != null) _objectValuesCollection.Add(key, value);
        }
        ...
        public void SaveUrl(string siteId)
        {
            ...
            if ((idObj != null) && Information.IsNumeric(idObj))
            {
                ...
                queryString = string.Join("&", queryString.Split('&').Where(s => !string.Equals(s,
                    $"{"browse_server_session_id"}={idObj}")).ToArray());
                url = string.IsNullOrEmpty(queryString) ? url : $"{url}?{queryString}";
                string sql =
                    $"if exists(select * from sysobjects where name = N'VE_URL') exec sp_executesql N'if exists(select * from ve_url where sid = @sid) update ve_url set url = @url where sid = @sid else insert into ve_url(sid, url) values(@sid, @url)', N'@url nvarchar(1024), @sid numeric', @url = '{GetSiteDns(siteId)}{url}', @sid = {idObj}";
                Cnn.ProcessData(sql);
            }
        }
