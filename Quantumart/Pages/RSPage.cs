...
namespace Quantumart.QPublishing.Pages
{
    ...
    public abstract class RSPage : Page
    {
        ...
        private readonly string[] _months = new string[] { "", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", 
        "Oct", "Nov", "Dec" };
        ...
        {
            return "_o" + Math.Max(System.Threading.Interlocked.Increment(ref _uniqueIdentifier), _uniqueIdentifier - 1);
        }
        ...
        private string _GetOperationsHtml()
        {
            ...
            Hashtable hashtable = _GetRSOperations();
            builder.Append("<ul>");
            foreach (string str in hashtable.Keys) {
                builder.Append("<li><a href=\"?op=" + str + "\">" + str + "</a>");
                string str2 = hashtable[str].ToString();
                if (!str2.Equals("")) {
        ...
        private string _GetRequestInfoHtml()
        {
            ...
            for (int m = 0; m <= length - 1; m++) {
                ...
                builder.Append("</td><td CLASS=\"xvalue\" WIDTH=\"70%\">");
                HttpCookie cookie = cookies.Get(allKeys[m]);
                if (cookie?.HasKeys ?? false) {
                    ...
                    if (cookie != null) builder.Append(cookie.Value);
                }
                builder.Append("</td></tr>");
            }
            if (length == 0) {
        ...
        private Hashtable _GetRSOperations()
        {
            ...
            for (int j = 0; j <= methods.Length - 1; j++) {
                object[] customAttributes = methods[j].GetCustomAttributes(false);
                if (customAttributes.Length > 0) {
                    bool flag = false;
                    string description = "";
                    for (int k = 0; k <= customAttributes.Length - 1; k++)
                    {
                    ...
                    if (flag) {
                        ...
                        for (int m = 0; m <= parameters.Length - 1; m++) {
        ...
        private void _GetServerProxy()
        {
            try {
                ...
                string str = _GetNextVariableName();
                Hashtable hashtable = _GetRSOperations();
                Hashtable hashtable2 = new Hashtable();
                foreach (string str2 in hashtable.Keys) {
                    string str3 = _GetNextVariableName();
        ...
        private void _InvokeMethod(string methodName, object[] parameters)
        {
            try {
                ...
                object o = GetType().InvokeMember(methodName, BindingFlags.InvokeMethod | BindingFlags.Public | BindingFlags.Instance, null, this, parameters);
                if (o is string) {
        ...
        private void _RunRemoteScriptingEngine()
        {
            ...
            if (request["pcount"] == null) {
                ...
                if (_writeDebugFile) {
                    try {
                        StreamWriter writer = File.CreateText(_debugFilepath);
        ...
        private void _ShowOperationResultPage()
        {
            ...
            if (method == null) {
                ...
                if (parameters.Length == 0) {
                    ...
                    object[] strArray = new object[-1 + 1];
                    try {
        ...
        private void _TestInvokeOperation(string methodName, object[] parameters)
        {
            ...
            Response.Write(parameters.Length > 0
        ...
        {
            MatchCollection matchs = new Regex("\\s*([A-Z]{1})\\w+").Matches(s);
            StringBuilder builder = new StringBuilder(matchs.Count);
