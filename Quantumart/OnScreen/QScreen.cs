...
namespace Quantumart.QPublishing.OnScreen
{
    ...
    public class QScreen
    {
        ...
        public static int AuthenticateForCustomTab(DBConnector cnn)
        {
            ...
            {
                string sql =
                    $"EXEC sp_executesql N'SELECT user_id from sessions_log WHERE sid = @sid', N'@sid nvarchar(255)', @sid = '{backendSid}'";
                var dt = cnn.GetRealData(sql);
                if (dt.Rows.Count > 0)
                {
                    result = (int)(decimal)dt.Rows[0]["user_id"];
                    sql =
                        $"EXEC sp_executesql N'UPDATE sessions_log SET sid = NULL WHERE sid = @sid', N'@sid nvarchar(255)', @sid = '{backendSid}'";
                    cnn.ProcessData(sql);
                }
            }
            return result;
            ...
            if (SessionEnabled()) {
                ...
                if (!string.IsNullOrEmpty(backendSid)) {
                    ...
                    if (!string.IsNullOrEmpty(backendUrl)) {
                        ...
                        string sql =
                            $" SELECT s.*, u.language_id, u.allow_stage_edit_object, u.allow_stage_edit_field FROM sessions_log AS s LEFT OUTER JOIN users AS u ON u.user_id = s.user_id WHERE sid='{backendSid}'";
        ...
        public int GetObjectTypeIdByObjectId(int objectId)
        {
            ...
            if (dt.Rows.Count > 0) {
                return DBConnector.GetNumInt(dt.Rows[0]["object_type_id"]);
            }
            else {
                return 0;
            }
        ...
        public static int GetAccessInternal(string table, int id, int uid, bool isUser, bool useDictionary)
        {
            ...
            if (Strings.LCase(table) == "content_item") {
                sql =
                    $" SELECT c.* FROM content_item AS i with(nolock) LEFT OUTER JOIN content AS c ON c.content_id = i.content_id WHERE i.content_item_id = {id}";
            ...
            if (result < 0) {
                ...
                //GetAccessInternal = Dictionary.DictionaryGet("PERMISSION_LEVEL", 0)
                //End If
            return result;
        }
        ...
        public static string GetBrowserInfo()
        {
            ...
            else if (agent.IndexOf("MSIE", StringComparison.Ordinal) >= 0) {
                return "ie";
            }
            else if (agent.IndexOf("Firefox", StringComparison.Ordinal) >= 0) {
                return "firefox";
            }
            else if (agent.IndexOf("Netscape/7", StringComparison.Ordinal) >= 0) {
                return "ns7";
            }
            else if (agent.IndexOf("Mozilla/5", StringComparison.Ordinal) >=0 && agent.IndexOf("Netscape", StringComparison.Ordinal) < 0) {
                return "mozilla";
            }
            else if (agent.IndexOf("Mozilla", StringComparison.Ordinal) >= 0) {
                return "ns";
            }
            else {
                return "unknown";
