...
(function () {
	var imageDialog = function (editor, dialogType) {
		...
		function onPopupClickHandler() {
		    var isContentLibrary = this.valueOf();
		    var dialog = CKEDITOR.dialog.getCurrent();
		    var $field = jQuery(dialog.getParentEditor().element.$);
		    var popup = isContentLibrary === true ? $field.data("imageWindow-content") : $field.data("imageWindow-site");
		    if (!popup) {
		        var eventArgs = new Quantumart.QP8.BackendEventArgs();
		        eventArgs.set_entityId(isContentLibrary === true ? $q.toInt($field.data("content_id"),0) : $q.toInt($field.data("library_entity_id"), 0));
		        eventArgs.set_parentEntityId($q.toInt($field.data("library_parent_entity_id"), 0));
		        eventArgs.set_entityTypeCode(isContentLibrary === true ? ENTITY_TYPE_CODE_CONTENT : ENTITY_TYPE_CODE_SITE);
		        eventArgs.set_actionCode(isContentLibrary === true ? ACTION_CODE_POPUP_CONTENT_LIBRARY : ACTION_CODE_POPUP_SITE_LIBRARY);
		        var currentZIndex = parseInt(jQuery(CKEDITOR.dialog.getCurrent().getElement().$).find(".cke_dialog").css("zIndex"), 10)

		        var options = {
		            "additionalUrlParameters": { "filterFileTypeId": Quantumart.QP8.Enums.LibraryFileType.Image, "subFolder": "" },
		            "zIndex": currentZIndex + 10,
		            "libraryUrl": $field.data('library_url')
		        };
		        popup = new Quantumart.QP8.LibraryPopupWindow(eventArgs, options);    
		        if (isContentLibrary === true) {	                
		            $field.data("imageWindow-content", popup);
		        }
		        else {	                
		            $field.data("imageWindow-site", popup);
		        }
					
		        popup.setSelectCallback(jQuery.proxy(function (url) {
		...
		return {
			...
			minWidth: 460,
			minHeight: 360,
			// всплывающее окно с библиотекой
			selectPopupWindowComponent: null,
			onShow: function () {
			...
			onHide: function () {
				...
				if (this.selectPopupWindowComponent) {
					this.selectPopupWindowComponent.dispose();
					this.selectPopupWindowComponent = null;
					this.selectPopupWindowComponent
				}
                                ...
                                // ------ QP Changes -------
                                {
                                    ...
                                    [
										{//site library
										    ...
										    //hidden : true,image.js
										    //filebrowser : 'info:txtUrl'
										    onClick: jQuery.proxy(onPopupClickHandler, false)
										},
                                    {//content library
                                        type: 'button',
                                        id: 'browseContent',
                                        ...
                                        style: 'display:inline-block;margin-top:10px;',
                                        align: 'center',
                                        label: editor.lang.common.browseContentLibrary,
                                        //hidden : true,image.js
                                        //filebrowser : 'info:txtUrl'
                                        onClick: jQuery.proxy(onPopupClickHandler, true)
                                    }
                                    ]
                                }
							]
