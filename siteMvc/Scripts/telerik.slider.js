(function ($) {
    ...
    $t.slider = function (element, options) {
        ...
        options.val = round($element.val() || round(options.val));
    ...
    $t.slider.Drag = function (options) {
        ...
        this.draggable = new $t.draggable({
            ...
            scope: options.element.id.replace('[', '').replace(']', ''),
    ...
    function createHtml(element, options) {
        ...
        if (inputs.length == 2) {
            inputs.eq(0).val(parseValue(options.selectionStart));
            inputs.eq(1).val(parseValue(options.selectionEnd));
        } else {
            $element.val(parseValue(options.val));
        }
    ...
    $t.rangeSlider = function (element, options) {
        ...
        inputs[0].type = "text";
        inputs[1].type = "text";
        options.selectionStart = round(inputs.eq(0).val() || options.selectionStart);
        options.selectionEnd = round(inputs.eq(1).val() || options.selectionEnd);
