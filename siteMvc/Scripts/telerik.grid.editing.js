(function ($) {
            ...
            if (grid.editing.mode == 'InCell') {
                ...
                grid.editCell = function (td) {
                    ...
                    if (grid.valid && (column && !column.readonly)) {
                        ...
                        if (grid.editing.insertRowPosition == "bottom") {
                            dataItem = grid.dataItem(tr) || grid.changeLog.get(index);
                        } else {
        ...
        deleteRow: function ($tr) {
            ...
            if (isPlainObject) {
                ...
                if (that.editing.mode != 'InCell') {
                    ...
                    that.changeLog.erase(that.rowIndex($tr), dataItem, $tr.hasClass("t-grid-new-row"));
            ...
            if (this.editing.mode != 'InCell') {
                ...
                if (this.editing.confirmDelete === false || confirm(this.localization.deleteConfirmation)) {
                    this.changeLog.erase(this.rowIndex($tr), dataItem, $tr.hasClass("t-grid-new-row"));
    ...
    $t.grid.CellBuilder = function (options) {
        function impl(dataItem, method) {
            ...
            return $.map(options.columns, function (column, index) {
                var classes = [];
                var attr = $t.splitClassesFromAttr(column.attr);

                if (index == editableIndex && method == "insert") {
                    classes.push("t-grid-edit-cell");
                } else if (index == options.columns.length - 1) {
                    classes.push("t-last");
                }

                if (attr.classes) {
                    classes.push(attr.classes);                    
                }

                return '<td ' + (attr.attributes ? attr.attributes : '') + (classes.length ? ' class="' + classes.join(" ") + '"' : "") + ">" +
    ...
    $t.grid.ChangeLog = function (size, insertRowPosition) {
        ...
        this.erase = function (index, values, isNew) {
            var inserted = this.inserted[index];
