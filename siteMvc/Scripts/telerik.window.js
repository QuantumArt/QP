    ...
    $t.window.prototype = {
        ...
        bringToTop: function () {
            var that = this,
                ...
                currentIndex = parseInt($(element).css('zIndex'), 10),
                zIndex = currentIndex,
                defaultIndex = 10001,
                allWindows = $(".t-window").filter(":visible");

            if (allWindows.length == 1 && $(element).is(":visible")) {
                return;
            }
            ...
            if (currentIndex == defaultIndex || zIndex > currentIndex) {
                $(element)
                    .css("zIndex", zIndex + 2)
