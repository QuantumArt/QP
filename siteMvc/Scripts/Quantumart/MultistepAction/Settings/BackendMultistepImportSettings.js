...
Quantumart.QP8.MultistepActionImportSettings.prototype = {
	...
	_initFileUploader: function (context, uploadPath) {
		...
		if (this._uploaderType == Quantumart.QP8.Enums.UploaderType.Silverlight)
			this._uploaderComponent = new Quantumart.QP8.BackendSilverlightUploader(this._fileWrapperElement, { background: "#ffffff", extensions: "", resolveName: this._renameMatch });
			...
			this._uploaderComponent = new Quantumart.QP8.BackendPlUploader(this._fileWrapperElement, {
				extensions: "", resolveName: this._renameMatch, useSiteLibrary: this._useSiteLibrary
			});
	...
	_initValidation: function () {
		...
		this._uniqueFieldToUpdatePredicate = function () { return +that._$importAction.val() > 0; };
		...
		if (this._$uniqueContentFieldId.length == 1) {
			this._$uniqueContentFieldId.on("change", function () { that._updateMappingOptions.call(that) });
			this._$uniqueFieldToUpdate.on("change", function () { that._updateMappingOptions.call(that) });
		}
	...
	loadFromFile: function (options, delim) {
		var that = this,
