...
Quantumart.QP8.BackendEntityEditor = function (editorGroupCode, documentWrapperElementId, entityTypeCode, entityId, actionTypeCode, options) {
	...
	// TODO: Пока так, потом должно приходить  с сервера
...
Quantumart.QP8.BackendEntityEditor.prototype = {
	...
	_initAllFields: function () {
		if (this._formElement) {
			...
			var self = this;
			if (this._customButtonsSettings) jQuery.each(this._customButtonsSettings, function (index, item) { self.addCustomButton(item) });
			if (this._customLinkButtonsSettings) jQuery.each(this._customLinkButtonsSettings, function (index, item) { self.addCustomLinkButton(item) });
	...
	_disposeVariationsModelField: function ($form) {
	...
	_disposeContextModelField: function ($form) {
	...
	_disposeErrorModelField: function ($form) {
	...
	isFieldValuesChanged: function () {
	    var $changedFields = jQuery(this._formElement).find(this.VARIATION_SELECTOR).closest("fieldset").find(this.REAL_CHANGED_EXCEPT_VARMODEL_SELECTOR);
	    return ($changedFields.length > 0);
	},
	...
	addCustomLinkButton: function (settings, $parent) {
		if (!settings.name && !settings.onClick && !settings.suffix && !settings.title)
			...
			builder.cat('<li><span id="' + options.id + '" class="linkButton actionLink">')
	...
	saveEntity: function (actionCode) {
		// Сохраняем данные всех визуальных редакторов
		$c.saveDataOfAllVisualEditors(this._formElement);
