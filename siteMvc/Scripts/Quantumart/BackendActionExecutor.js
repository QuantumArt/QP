...
//#region class BackendActionParameters
// === Класс "Параметры действия" ===
Quantumart.QP8.BackendActionParameters = function (options) {
	Quantumart.QP8.BackendActionParameters.initializeBase(this);

	if ($q.isObject(options.eventArgs)) { // инициализация параметров по eventArgs
	...
	else { // обычная инициализация параметров
		if (options.entities) {
...
Quantumart.QP8.BackendActionParameters.prototype = {
	...
	correct: function (action) {
		...
		if (action.Code == ACTION_CODE_CONTENT_PERMISSIONS_FOR_CHILD || action.Code == ACTION_CODE_ARTICLE_PERMISSIONS_FOR_CHILD) {
			this._parentEntityId = this._entityId;
			this._entityId = 0;
			if (action.Code == ACTION_CODE_CONTENT_PERMISSIONS_FOR_CHILD) {
				this._actionCode = ACTION_CODE_CONTENT_PERMISSIONS;
			}
			else if (action.Code == ACTION_CODE_ARTICLE_PERMISSIONS_FOR_CHILD) {
				this._actionCode = ACTION_CODE_ARTICLE_PERMISSIONS;
			}
		}

		if (action.Code == ACTION_CODE_ADD_NEW_ADJACENT_FIELD)
		{
		    this._context = this._context || {};
		    jQuery.extend(this._context, { additionalUrlParameters: { fieldId: this._entityId } });
		    this._entityId = 0;
		}


		if (this._entityTypeCode != currentAction.EntityType.Code && currentAction.ActionType.Code != ASSEMBLE_PARENT_ACTION_TYPE) {
			this._entityTypeCode = currentAction.EntityType.Code;
			if (this._entityTypeCode != ENTITY_TYPE_CODE_SITE_FILE && this._entityTypeCode != ENTITY_TYPE_CODE_CONTENT_FILE) {
			    if (!this._isGroup) 
			        this._parentEntityId = this._entityId;
			    else {
			        this._context = this._context || {};
			        jQuery.extend(this._context, { additionalUrlParameters: { groupId: this._entityId } });
			    }
