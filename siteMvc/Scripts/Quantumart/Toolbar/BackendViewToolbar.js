...
var EVENT_TYPE_VIEW_TOOLBAR_PREVIEW_BUTTON_CLICKING = "OnViewToolbarPreviewButtonClicking";
var EVENT_TYPE_VIEW_TOOLBAR_PREVIEW_BUTTON_CLICKED = "OnViewToolbarPreviewButtonClicked";
var EVENT_TYPE_VIEW_TOOLBAR_SEARCH_BUTTON_CLICKING = "OnViewToolbarSearchButtonClicking";
var EVENT_TYPE_VIEW_TOOLBAR_SEARCH_BUTTON_CLICKED = "OnViewToolbarSearchButtonClicked";
...
Quantumart.QP8.BackendViewToolbar.prototype = {
 ...
 result = jQuery("li.selected", $list).attr("code"); 
	...
	_getToolbarItemCollectionFromAction: function (action) {
	    ...
	    // Добавляем кнопку "Показать/скрыть панель переключение контекста"
		...
		// Добавляем расскрывающийся список "Представления"
		...
		var viewsDropDown = {
			Type: TOOLBAR_ITEM_TYPE_DROPDOWN,
		...
		if (!$q.isNullOrEmpty(action.Views)) {
			jQuery.each(action.Views,
				function (index, actionView) {
					...
					if (view.Code != VIEW_TYPE_CODE_TREE || $o.checkEntityForPresenceSelfRelations(parentEntityTypeCode, self.get_parentEntityId())) {
		...
		// Добавляем кнопку "Показать/скрыть панель предварительного просмотра"
		...
		// Добавляем кнопку "Показать/скрыть панель поиска"
	...
	notifyToolbarButtonClicking: function (eventArgs) {
		...
		if (itemValue == this.PREVIEW_BUTTON_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_PREVIEW_BUTTON_CLICKING, newArgs);
		} else if (itemValue == this.SEARCH_BUTTON_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_SEARCH_BUTTON_CLICKING, newArgs);
		} else if (itemValue == this.CONTEXT_BUTTON_CODE) {
		    this.notify(EVENT_TYPE_VIEW_TOOLBAR_CONTEXT_BUTTON_CLICKING, newArgs);
		}
	...
	notifyToolbarButtonClicked: function (eventArgs) {
		...
		if (itemValue == this.PREVIEW_BUTTON_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_PREVIEW_BUTTON_CLICKED, newArgs);
		} else if (itemValue == this.SEARCH_BUTTON_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_SEARCH_BUTTON_CLICKED, newArgs);
		} else if (itemValue == this.CONTEXT_BUTTON_CODE) {
		    this.notify(EVENT_TYPE_VIEW_TOOLBAR_CONTEXT_BUTTON_CLICKED, newArgs);
		}
	...
	notifyDropDownSelectedIndexChanging: function (eventArgs) {
		...
		if (itemValue == this.VIEWS_DROPDOWN_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_VIEWS_DROPDOWN_SELECTED_INDEX_CHANGING, newArgs);
		}
	...
	notifyDropDownSelectedIndexChanged: function (eventArgs) {
		...
		if (itemValue == this.VIEWS_DROPDOWN_CODE) {
			this.notify(EVENT_TYPE_VIEW_TOOLBAR_VIEWS_DROPDOWN_SELECTED_INDEX_CHANGED, newArgs);
		}
