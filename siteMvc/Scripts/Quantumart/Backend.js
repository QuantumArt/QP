...
Quantumart.QP8.Backend = function (isDebugMode, options) {
	...
	jQuery(document).bind("click", function (e) { if (e.which == 2) e.preventDefault(); });
...
Quantumart.QP8.Backend.prototype = {
	...
	_initialize: function () {
		...
		this._directLinkExecutor.ready(jQuery.proxy(function (openByDirectLink) {
			...
			this._backendTreeMenu = new Quantumart.QP8.BackendTreeMenu("MainTreeMenu",
						{ "treeContainerElementId": "tree", "contextMenuManager": this._backendTreeMenuContextMenuManager });
	...
	_onDragStartSplitter: function (eventType, sender, eventArgs) {
        if (!this._busy)
	        this._backendEditingArea.showAjaxLoadingLayer();
	},

	_onDropSplitter: function (eventType, sender, eventArgs) {
        if (!this._busy)
	...
	_isMultistep: function (action, eventArgs) {
	    ...
	    var isMultistep =
             action.IsMultistep ||
            (
                action.AdditionalControllerActionUrl &&
		...
		if (status == BACKEND_ACTION_EXECUTION_STATUS_NOT_STARTING) {
		    ...
		    if ($q.isObject(action)) {
		        if (action.IsInterface) {
		            ...
		            jQuery.when(this._backendActionExecutor.executeMultistepAction(eventArgs))
		            ...
		            this._markAsBusy();
		            var callback = jQuery.proxy(
						function (status, eventArgs) {
						    if (status == BACKEND_ACTION_EXECUTION_STATUS_SUCCESS) {
