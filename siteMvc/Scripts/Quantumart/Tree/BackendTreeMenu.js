...
//#region event types of tree menu
// === Типы событий дерева ===
var EVENT_TYPE_TREE_MENU_ACTION_EXECUTING = 'OnTreeMenuActionExecuting';
var EVENT_TYPE_TREE_MENU_NODE_REMOVING = 'OnTreeMenuNodeRemoving';
var EVENT_TYPE_TREE_MENU_NODE_REMOVED = 'OnTreeMenuNodeRemoved';

//#endregion

//#region class BackendTreeMenu
// === Класс "Древовидное меню" ===
...
Quantumart.QP8.BackendTreeMenu.prototype = {
  _contextMenuManagerComponent: null, // компонент "Менеджер контекстных меню"
  ...
  initialize: function() {
    ...
    var $tree = jQuery(this._treeComponent.element);

    if (this._contextMenuManagerComponent != null) {
      var contextMenuEventType = this._contextMenuManagerComponent.getContextMenuEventType();

      $tree
              .delegate(this.NODE_NEW_CLICKABLE_SELECTORS, contextMenuEventType, this._onContextMenuHandler);
    }

    $tree = null;

    if (this._contextMenuManagerComponent != null) {
      this._contextMenuManagerComponent.attachObserver(EVENT_TYPE_CONTEXT_MENU_SHOWING, this._onNodeContextMenuShowingHandler);
      this._contextMenuManagerComponent.attachObserver(EVENT_TYPE_CONTEXT_MENU_ITEM_CLICKING, this._onNodeContextMenuItemClickingHandler);
      this._contextMenuManagerComponent.attachObserver(EVENT_TYPE_CONTEXT_MENU_HIDDEN, this._onNodeContextMenuHiddenHandler);
    }
  ...
  fixTreeHeight: function(leftSplitterPaneHeight) {
    if (!$q.isNull(leftSplitterPaneHeight)) {
      ...
      if (leftSplitterPaneHeight > 0) {
        jQuery('#tree').css('height', leftSplitterPaneHeight + 'px');
  ...
  isTreeBusy: function() {
    var $tree = jQuery(this._treeComponent.element);
    var isBusy = $tree.find('UL:first').hasClass(this.NODE_BUSY_CLASS_NAME);
  ...
  addNodesToParentNode: function(parentNode, maxExpandLevel) {
    this._addNodesToParentNode(parentNode, maxExpandLevel,
  ...
  _refreshNodeInner: function($node, loadChildNodes, callback) {
    ...
    Quantumart.QP8.BackendTreeMenu.getTreeMenuNode(entityTypeCode, entityId, parentEntityId, isFolder, isGroup, groupItemCode, loadChildNodes,
      function(data) {
  var node = data;

  if (node) {
  // Обновляем информации об узле
  ...
  highlightNodeWithEventArgs: function(eventArgs) {
    ...
    if (!$q.isNullOrEmpty($highlightedNode)) {
      ...
      if (!!eventArgs.isExpandRequested) {
        ...
        Quantumart.QP8.BackendTreeMenu.getSubTreeToEntity(eventArgs.get_entityTypeCode(), eventArgs.get_parentEntityId(), eventArgs.get_entityId(),
                    function(data) {
                      if (!!data) {
                        ...
                        var _exp = function(node) {
                          if (!node || $q.isNullOrEmpty(self.getNode(self.generateNodeCode(node.Code, node.Id, node.ParentId, node.IsFolder)))) {
                            ...
                            return _exp(jQuery.grep(node.ChildNodes, function(n) {
  ...
  _expandToEntityNode: function(entityTypeCode, parentEntityId, entityId) {
    ...
    Quantumart.QP8.BackendTreeMenu.getSubTreeToEntity(entityTypeCode, parentEntityId, entityId,
            function(data) {
              if (!!data) {
                  // найти самый нижний node из уже существующих в дереве
                var findDeepest = function(node, toExpand) {
                  if (!node || $q.isNullOrEmpty(self.getNode(self.generateNodeCode(node.Code, node.Id, node.ParentId, node.IsFolder)))) {
                    ...
                    return findDeepest(jQuery.grep(node.ChildNodes, function(n) {
                      return n.ChildNodes != null;
                    })[0], toExpand) || node;
                  }
                };
                ...
                var findChildren = function(options, parent) {
                  if (!parent || $q.isNullOrEmpty(parent.ChildNodes)) {
                    return [];
                      ...
                      // условие того что текущий parent являеться искомым
                    ...
                    return findChildren(options, jQuery.grep(parent.ChildNodes || [], function(n) {
                      return n.ChildNodes != null;
                    })[0]);
                  }
                };
                ...
                if (!!deepestExistedNode) {
                  ...
                  self._addNodesToParentNode($node, 100,
                            function(options, successHandler) {
                              successHandler(
                                  findChildren(options, deepestExistedNode).slice(0) // Важно, так как _addNodesToParentNode модифицирует аргумент
                              );
                            },
                            function() { }
                        );

                        // выделить запрашиваемый node
  ...
  scrollToNode: function(node) {
    ...
    if (!$q.isNullOrEmpty($node)) {
      jQuery('#' + this._treeContainerElementId).scrollTo($node, { offset: -100, duration: 300, axis: 'y' });
    }
  ...
  _extendNodeElement: function(nodeElem, node) {
    ...
    $node.data('entity_type_code', node.Code);
    $node.data('entity_id', node.Id);
    $node.data('entity_name', node.Title);
    $node.data('parent_entity_id', node.ParentId);
    $node.data('is_folder', node.IsFolder);
    $node.data('is_group', node.IsGroup);
    $node.data('group_item_code', node.GroupItemCode);
    $node.data('default_action_code', node.DefaultActionCode);
    $node.data('default_action_type_code', node.DefaultActionTypeCode);
    $node.data('context_menu_code', node.ContextMenuCode);
    ...
    if (!$q.isNullOrWhiteSpace(contextMenuCode) && this._contextMenuManagerComponent) {
      ...
      if ($q.isNullOrEmpty(contextMenu)) {
        ...
        contextMenu = this._contextMenuManagerComponent.createContextMenu(
          contextMenuCode, String.format('treeContextMenu_{0}', contextMenuCode),
          { targetElements: targetElems, allowManualShowing: true });
  ...
  executeAction: function(node, actionCode) {
    ...
    if (!$q.isNullOrEmpty($node)) {
      ...
      if (!action) {
        ...
        if (!isCustomAction && actionTypeCode == ACTION_TYPE_CODE_REFRESH) {
        ...
        this.notify(EVENT_TYPE_TREE_MENU_ACTION_EXECUTING, eventArgs);
  ...
  _onNodeClicking: function(e) {
    ...
    if (!this.isTreeBusy()) {
      ...
      if (!$node.data('is_folder') && this._isNodeCollapsed($node)) {
        this._treeComponent.nodeToggle(null, $node, true);
      }

      if (!$q.isNullOrWhiteSpace($node.data('default_action_code'))) {
        this.executeAction($node, $node.data('default_action_code'));
      }
    } else {
      $node = null;
      e.preventDefault();
  ...
  dispose: function() {
    ...
    $tree.delegate(this.NODE_NEW_CLICKABLE_SELECTORS, 'click', this._onNodeClickingHandler);
    ...
    if (this._contextMenuManagerComponent != null) {
      ...
      this._contextMenuManagerComponent.detachObserver(EVENT_TYPE_CONTEXT_MENU_SHOWING, this._onNodeContextMenuShowingHandler);
      this._contextMenuManagerComponent.detachObserver(EVENT_TYPE_CONTEXT_MENU_ITEM_CLICKING, this._onNodeContextMenuItemClickingHandler);
      this._contextMenuManagerComponent.detachObserver(EVENT_TYPE_CONTEXT_MENU_HIDDEN, this._onNodeContextMenuHiddenHandler);
...
Quantumart.QP8.BackendTreeMenu.getTreeMenuNode = function(entityTypeCode, entityId, parentEntityId, isFolder, isGroup, groupItemCode, loadChildNodes, successHandler, errorHandler) {
  $q.getJsonFromUrl(
  'GET',
  CONTROLLER_URL_TREE_MENU + 'GetNode',
    {
  entityTypeCode: entityTypeCode,
  entityId: entityId,
  parentEntityId: parentEntityId,
...
Quantumart.QP8.BackendTreeMenu.getTreeMenuChildNodesList = function(options, successHandler, errorHandler) {
  $q.getJsonFromUrl(
    'GET',
    CONTROLLER_URL_TREE_MENU + 'GetChildNodesList',
    {
...
Quantumart.QP8.BackendTreeMenu.getSubTreeToEntity = function(entityTypeCode, parentEntityId, entityId, successHandler) {
  $q.getJsonFromUrl(
    'GET',
    CONTROLLER_URL_TREE_MENU + 'GetSubTreeToEntity',
    {
  entityTypeCode: entityTypeCode,
  parentEntityId: parentEntityId,
  entityId: entityId
