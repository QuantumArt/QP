...
Quantumart.QP8.BackendEntityDataListBase.prototype = {
	...
	_createCollapsingToolbar: function (mainWrapperElement) {
		if (mainWrapperElement) {
			...
			jQuery(mainWrapperElement)
				.prepend($collapsingToolbar)
				.find('.' + this.LIST_WRAPPER_CLASS_NAME)
					.hide();
	...
	_extendLink: function ($linkButton, actionCode) {
		if (!$q.isNullOrEmpty($linkButton)) {
			...
			if (actionTypeCode == ACTION_TYPE_CODE_ADD_NEW && entityTypeCode == ENTITY_TYPE_CODE_ARTICLE) {
	...
	_loadSelectedItems: function (entities) {
		if (($o.checkEntitiesForPresenceEmptyNames(entities) || this._readDataOnInsert) && entities.length <= this._countLimit) {
			...
			dataItems = $o.getSimpleEntityList(this._entityTypeCode, this._parentEntityId, this._entityId,
							this._listId, this._selectionMode, selectedEntitiesIDs, this._filter, 0);
		}
		else {
	...
	_getCountDivHtml: function(newCount, hidden) {
		...
		html
			.cat('<div class="countItemsBlock" ' + style + '>')
            .cat(countText)
			.cat(overflowText)
            .cat('</div>')
	    ;
