...
Quantumart.QP8.BackendArticleSearchBlock.FieldSearchContainer.prototype = {
	...
	// возвращает размер окна для фильтра заданного типа
	...
	_onCloseAndApplyWndClick: function () {
	    ...
	    jQuery(this._containerElement).closest('form').trigger("submit");
	},

	_onFilterFormSubmitted: function (e) {
	    e.preventDefault();
	    jQuery(this._closeAndApplyButton).trigger("click");
	    return false;
	},
	...
	dispose: function () {
		...
		jQuery(this._closeButtonElement).unbind("click");
        jQuery(this._contentWindowOpenLinkElement).unbind("click");

        if (this._popupWindowComponent) {
            jQuery(this._popupWindowComponent.element).unbind("close");
		    jQuery(".closeFilter", this._popupWindowComponent.element).unbind("click");
			jQuery(".closeAndApplyFilter", this._popupWindowComponent.element).unbind("click");
            jQuery("form", this._popupWindowComponent.element).unbind("submit");
