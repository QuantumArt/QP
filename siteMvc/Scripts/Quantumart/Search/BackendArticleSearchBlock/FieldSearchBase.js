...
Quantumart.QP8.BackendArticleSearchBlock.FieldSearchBase.prototype = {
    ...
    if (callback) ids = ids.map(callback);
    var result = ids.join("; ");
    if (length > count) result += "; " + $l.SearchBlock.etsText + ", " + $l.SearchBlock.totalText + ": " + length;
    return result;
    },
    ...
    _getIds: function (text) {
        ...
        ids = jQuery.grep(ids, function (e) { return (e) });
        ids = _.uniq(ids);
        return ids;
    }
};
...
//#region class BackendArticleSearchBlock.FieldSearchState
