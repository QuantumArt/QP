...
Quantumart.QP8.BackendSearchBlockBase.prototype = {
	...
	initialize: function () {
		var $searchBlock = jQuery("#" + this._searchBlockElementId);

		var searchBlockExist = !$q.isNullOrEmpty($searchBlock);
		var searchFormExist = !$q.isNullOrEmpty($searchBlock.find("FORM"));

		if (!searchBlockExist) {
			$searchBlock = jQuery("<div />", { "id": this._searchBlockElementId, "class": "searchBlock", "css": { "display": "none"} });
		}

		if (!searchFormExist) {
			var $searchForm = jQuery("<form />", { "class": "formLayout" });
			$searchBlock.append($searchForm);

			var $concreteSearchBlock = jQuery("<div />");
			$searchForm.append($concreteSearchBlock);

			var $buttonsWrapper = jQuery("<div />")
			if (this._hideButtons) {
				$buttonsWrapper.hide();
			}
			$searchForm.append($buttonsWrapper);

			var $findButton = jQuery("<input />", { "type": "button", value: $l.SearchBlock.findButtonText, "class": "button" });
			$buttonsWrapper.append($findButton);

			var $resetButton = jQuery("<input />", { "type": "button", value: $l.SearchBlock.resetButtonText, "class": "button" });
		...
		if (!searchBlockExist) {
			if (!$q.isNullOrWhiteSpace(this._searchBlockContainerElementId)) {
				jQuery("#" + this._searchBlockContainerElementId).append($searchBlock);
			}
			else {
				jQuery("BODY:first").append($searchBlock);
			}
		}
	...
	_attachSearchBlockEventHandlers: function () {
		...
		$searchBlock.bind("resize", this._onSearchBlockResizedHandler);

		$searchBlock = null;

		var $findButton = jQuery(this._findButtonElement);
		$findButton.bind("click", this._onFindButtonClickHandler);

		$findButton = null;

		var $resetButton = jQuery(this._resetButtonElement);
		$resetButton.bind("click", this._onResetButtonClickHandler);

		$resetButton = null;

		var $searchForm = jQuery(this._searchForm);
		$searchForm.bind("submit", this._onSearchFormSubmittedHandler);
	...
	_detachSearchBlockEventHandlers: function () {
		var $searchBlock = jQuery(this._searchBlockElement);
		$searchBlock.unbind("resize", this._onSearchBlockResizedHandler);

		$searchBlock = null;

		var $findButton = jQuery(this._findButtonElement);
		$findButton.unbind("click", this._onFindButtonClickHandler);

		$findButton = null;

		var $resetButton = jQuery(this._resetButtonElement);
		$resetButton.unbind("click", this._onResetButtonClickHandler);
	...
	showSearchBlock: function () {
		...
		if ($searchBlock.is(":hidden")) {
			...
			$searchBlock
				.show()
				.height(this._lastSearchBlockHeight)
				.trigger("resize");
		}
	},
