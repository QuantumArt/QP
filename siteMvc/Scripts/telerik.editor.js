...
function domToXhtml(root) {
    ...
    function attr(node) {
        ...
        if (dom.is(node, "iframe")) {
            var $node = $(node);
            if ($node.attr("originalsrc") != "") {
                $node.attr("src", $node.attr("originalsrc"));
                $node.removeAttr("originalsrc");
            }
        }
...
$.extend($t.editor, {
    ...
    ViewHtmlCommand: ViewHtmlCommand,
...
$.fn.tEditor.defaults = {
    ...
    tools: {
        ...
        viewHtml: new Tool({ command: ViewHtmlCommand })
    }
}
